...
@app.route('/api/stats/local', methods=['GET'])
def api_stats_local():
    local_stats = {
        "global_captions": 0,
        "categories": []
    ...
    return jsonify({"status": "success", "local": local_stats})

@app.route('/api/stats/tumblr', methods=['GET'])
def api_stats_tumblr():
    fresh_stats = get_tumblr_stats()
    if fresh_stats:
        analytics_db.save_stats(fresh_stats['followers'], fresh_stats['total_posts'], fresh_stats['queue_length'])
    ...
    return jsonify({"status": "error", "message": "Tumblr API unavailable"}), 500
